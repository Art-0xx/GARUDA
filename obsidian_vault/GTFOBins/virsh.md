...
```
_This requires the user to be in the `libvirt` group. If the target directory doesn't exist, `pool-create-as` must be run with the `--build` option. The destination file ownership and permissions can be set in the XML._
...
virsh -c qemu:///system pool-destroy x
```
_This requires the user to be in the `libvirt` group._
**Contexts:** sudo, unprivileged
