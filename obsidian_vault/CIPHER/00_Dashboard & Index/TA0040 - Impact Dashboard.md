...
## 3. Actors Using Impact Techniques
```dataviewjs
...
dv.table(["Threat Actor","# Impact Techniques","Techniques"],
...
## 4. Malware / Tools Used in Impact
```dataviewjs
...
dv.table(["Malware / Tool","# Impact Techniques","Techniques"],
  Object.values(mal).sort((a,b)=>b.count-a.count)
  .map(x=>[x.ref,x.count,x.techs.join(", ")]));

```

## 5. Campaigns Using Impact Techniques
```dataviewjs
...
dv.table(["Campaign","# Impact Techniques","Techniques"],
  Object.values(camps).sort((a,b)=>b.count-a.count)
  .map(x=>[x.ref,x.count,x.techs.join(", ")]));

```

## 6. Incomplete Impact Techniques
```dataview
