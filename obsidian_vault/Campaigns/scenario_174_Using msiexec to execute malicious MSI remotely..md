...
# Using msiexec to execute malicious MSI remotely.
...
## Detection Logic
Monitor msiexec remote: `wmic process where name='msiexec.exe' get commandline | findstr http`; YARA rule: `rule MsiexecRemote { strings: $a = /msiexec.*http.*msi/ nocase; condition: $a }`.
