...
# Using InstallUtil.exe to execute malicious .NET binary via HTTP.
...
## Detection Logic
Monitor InstallUtil HTTP: `wmic process where name='InstallUtil.exe' get commandline | findstr http`; YARA rule: `rule InstallUtilHttp { strings: $a = /InstallUtil.*http/ nocase; condition: $a }`.
...
## AI Training Prompt
Train AI to detect InstallUtil HTTP execution and suggest firewall rules.
